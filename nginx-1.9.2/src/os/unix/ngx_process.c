    ...
    char *name, ngx_int_t respawn) //respawn取值为NGX_PROCESS_RESPAWN等，或者为进程在ngx_processes[]中的序号
{
    ...
    if (respawn >= 0) {  //替换进程ngx_processes[respawn],可安全重用该进程表项  
        ...
        for (s = 0; s < ngx_last_process; s++) { 
            if (ngx_processes[s].pid == -1) { //先找到一个被回收的进程表Item
                break;
            }
        }
    ...
    if (respawn != NGX_PROCESS_DETACHED) { //不是分离的子进程      /* 不是热代码替换 */
        ...
        /* 
          ...
          这个方法可以创建一对关联的套接字sv[2]。下面依次介绍它的4个参数：参数d表示域，在Linux下通常取值为AF UNIX；type取值为
          SOCK_STREAM或者SOCK_DGRAM，它表示在套接字上使用的是TCP还是UDP; protocol必须传递0；sv[2]是一个含有两个元素的整型数组，
          实际上就是两个套接字。当socketpair返回0时，sv[2]这两个套接字创建成功，否则socketpair返回一1表示失败。
          注意socketpair的协议族为AF_UNIX UNXI域
             当socketpair执行成功时，sv[2]这两个套接字具备下列关系：
             向sv[0]套接字写入数据，将可以从sv[l]套接字中读取到刚写入的数据； 同样，向sv[l]套接字写入数据，也可以从sv[0]中读取到写入的数据。
             通常，在父、子进程通信前，会先调用socketpair方法创建这样一组套接字，
             在调用fork方法创建出子进程后，
             将会在父进程中关闭sv[1]套接字，仅使用sv[0]套接字用于向子进程发送数据(以及接收子进程发送来的数据)
             而在子进程中则关闭sv[0]套接字，仅使用sv[1]套接字接收父进程发来的数据(也可以向父进程发送数据)
             Master ----------> Write sv[0] ------ Socketpair ------- Read sv[1] ---------------> Worker
          */  
