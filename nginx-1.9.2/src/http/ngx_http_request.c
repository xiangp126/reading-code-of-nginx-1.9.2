...
// 当建立连接后开辟ngx_http_connection_t结构
// 这里面存储该服务器端ip:port所在server{}上下文配置信息，和server_name信息等
// 然后让ngx_connection_t->data指向该结构
// 这样就可以通过ngx_connection_t->data获取到服务器端的serv loc 等配置信息以及该server{}中的server_name信息
void
ngx_http_init_connection(ngx_connection_t *c) 
{
    ...
    // 在服务器端accept客户端连接成功(ngx_event_accept)后
    // 会通过ngx_get_connection从连接池获取一个ngx_connection_t结构
    ...
    if (port->naddrs > 1) {  
         ...
         */
        // 说明listen ip:port存在通配符配置，如listen *:port
        // 那么就需要通过ngx_connection_local_sockaddr来确定究竟客户端是和那个本地ip地址建立的连接
    ...
    // 在ngx_event_accept的时候才会置1
    // 才可能执行下面的if里面的内容，否则不会执行if里面的内容
    if (rev->ready) {
...
/*
在有些情况下，当TCP连接建立成功时同时也出现了可读事件
（例如，在套接字listen配置时设置了deferred选项时，内核仅在套接字上确实收到请求时才会通知epoll
...
也就是说，如果经过client_header_timeout时间后这个连接上还没有用户数据到达
...
// 因此当客户端连接建立成功后，只有第一次读取客户端数据才会走该函数
        ...
        if (rc == NGX_OK) { //请求行解析成功
            ...
            if (r->http_version < NGX_HTTP_VERSION_10) { //1.0以下版本没有请求头部字段，
                /*
                    ...
                    头部这一步骤。这时将会调用ngx_http_find_virtual_server方法寻找到相应的虚拟主机?1?7
                    */
