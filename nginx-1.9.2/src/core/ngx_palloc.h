...
*/ //内存块包含的数据   
typedef struct {
    u_char               *last;//当前内存池节可申请的首地址    pool->d.last ~ pool->d.end 中的内存区便是可用数据区。
...
内存块数据       ---  ngx_pool_data_t;
大内存块         --- ngx_pool_large_s; 
图形化理解参考Nginx 内存池（pool）分析 http://www.linuxidc.com/Linux/2011-08/41860.htm
*/

struct ngx_pool_s {
    ngx_pool_data_t       d;    //节点数据 包含 pool 的数据区指针的结构体 pool->d.last ~ pool->d.end 中的内存区便是可用数据区。
    size_t                max;  //当前pool可以申请的最大内存空间,一次从pool中开辟的最大空间
